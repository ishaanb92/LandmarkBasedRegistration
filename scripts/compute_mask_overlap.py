...
if __name__ == '__main__':
    ...
    for idx, pdir in enumerate(pat_dirs):
        ...
        resampled_mask_np = sitk.GetArrayFromImage(resampled_mask_itk)

        dice = np.sum(resampled_mask_np[fixed_mask_np==1])*2.0 / (np.sum(fixed_mask_np) + np.sum(resampled_mask_np))
